...
app.post("/settings/personal-details", authenticateToken, async (req, res) => {
  ...
  const updateOrInsertSetting = (key, value) => new Promise((resolve, reject) => {
    db.get("SELECT * FROM settings WHERE user_id = ? AND setting_key = ?", [userId, key], (err, row) => {
      ...
      if (row) {
        db.run("UPDATE settings SET setting_value = ? WHERE user_id = ? AND setting_key = ?", [value, userId, key], (err) => {
          if (err) reject(err);
          resolve();
        });
      } else {
        db.run("INSERT INTO settings (user_id, setting_key, setting_value) VALUES (?, ?, ?)", [userId, key, value], (err) => {
          if (err) reject(err);
          resolve();
        });
      }
  ...
  try {
    ...
    db.get("SELECT * FROM watchlist WHERE user_id = ? AND collection_slug = ?", [userId, slug], (err, row) => {
      ...
      if (row) {
        ...
        db.run("INSERT INTO watchlist (user_id, collection_slug, collection_name, set_price) VALUES (?, ?, ?, ?)", [userId, slug, name, null], (err) => {
          if (err) {
            throw err;
          }
...
app.delete("/watchlist/delete", authenticateToken, (req, res) => {
  ...
  db.run("DELETE FROM watchlist WHERE id = ? AND user_id = ?", [id, userId], (err) => {
    if (err) {
      return res.status(500).json({error: err.message});
    }
...
app.put("/watchlist/edit", authenticateToken, (req, res) => {
  ...
  db.run("UPDATE watchlist SET set_price = ? WHERE id = ? AND user_id = ?", [set_price, id, userId], (err) => {
    if (err) {
      return res.status(500).json({error: err.message});
    }
    db.get(
  ...
  try {
    ...
    db.get("SELECT * FROM galleries WHERE name = ? AND user_id = ?", [name, userId], (err, row) => {
      ...
      if (row) {
        ...
        // Collection does not exist, insert it
        db.run("INSERT INTO galleries (user_id, name, description) VALUES (?, ?, ?)", [userId, name, description], (err) => {
          if (err) {
            throw err;
          }
...
app.put("/galleries/edit", authenticateToken, (req, res) => {
  ...
  db.run("UPDATE galleries SET name = ?, description = ? WHERE id = ? AND user_id = ?", [name, description, id, userId], (err) => {
    if (err) {
      return res.status(500).json({error: err.message});
    }
    db.get(
...
app.delete("/galleries/delete", authenticateToken, (req, res) => {
  ...
  db.run("DELETE FROM galleries WHERE id = ? AND user_id = ?", [id, userId], (err) => {
    if (err) {
      return res.status(500).json({error: err.message});
    }
    return res.status(200).json({success: true, id});
  ...
  try {
    ...
    db.get("SELECT * FROM gallery_items WHERE gallery_id = ? AND contract_addr = ? AND token_id = ?", [gallery_id, contract_addr, token_id], (err, row) => {
      ...
      if (row) {
        ...
        // Collection does not exist, insert it
        db.run("INSERT INTO gallery_items (gallery_id, contract_addr, token_id, collection_name) VALUES (?, ?, ?, ?)", [gallery_id, contract_addr, token_id, collection_name], (err) => {
          if (err) {
            throw err;
          }
...
app.delete("/gallery-items/delete", authenticateToken, (req, res) => {
  ...
  db.run("DELETE FROM gallery_items WHERE id = ?", [id], (err) => {
    if (err) {
      return res.status(500).json({error: err.message});
    }
    return res.status(200).json({success: true, id});
